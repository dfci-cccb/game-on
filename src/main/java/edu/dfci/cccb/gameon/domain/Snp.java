...
public class Snp {
  ...
  private static final LoadingCache<UserDetails, AtomicInteger> users =
                                                                        CacheBuilder.newBuilder ()
                                                                                    .expireAfterWrite (DURATION,
                                                                                                       DURATION_UNIT)
                                                                                    .build (new CacheLoader<UserDetails, AtomicInteger> () {
                                                                                      public AtomicInteger load (UserDetails key) {
                                                                                        return new AtomicInteger (1);
                                                                                      }
                                                                                    });
  private static final LoadingCache<String, List<String>> distinctValues =
                                                                           CacheBuilder.newBuilder ()
  ...
  public static TypedQuery<Snp> findGeneric (Map<String, String[]> searchTerms) throws QueryLimitExceededException {
    try {
      if (users.get ((UserDetails) SecurityContextHolder.getContext ()
                                                        .getAuthentication ()
                                                        .getPrincipal ()).incrementAndGet () > MAXIMUM_QUERIES)
  ...
  private static String formatWhereClause (final Map<String, String[]> searchTerms,
